...
export default function Fulfillments() {
    ...
    const [filterOrders, setFilterOrders] = useState<OrderReturn[]>([]);
    ...
    const getOrderReturns = async () => {
        try {
            const res = await request<OrderReturn[]>({
                method: 'GET',
    ...
    const fetchEbayReturns = async () => {
        try {
            await request({
                method: 'GET',
                url: '/ebay/orderreturn/fetch-all',
            });
    ...
    return (
        ...
        <div className="flex-1 p-6 lg:pl-8 lg:pr-8 lg:pt-8 lg:pb-8">
            <h1 className="text-3xl font-bold text-foreground mb-6">Order Returns</h1>

            <div className="flex space-x-4 mb-6">
                <div className='w-1/2'>
                    <Link href="/order-return" className='inline-block py-2 px-5 border bg-blue-600 text-white rounded'>Return</Link>
                    <Link href="/order-refund" className='inline-block py-2 px-5 border mx-5 bg-gray-200 text-gray-700 rounded'>Refund</Link>
                </div>

                <div className='w-1/2 text-right'>
                    <button className=' py-2 px-5 border' onClick={() => { fetchEbayReturns() }}>Fetch Returns</button>
                </div>
            </div>
                    ...
                    <tbody>
                        {filterOrders.length > 0 && filterOrders.map((returnOrder) => {
                            return (<tr key={returnOrder.id}>
                                ...
                                <td className='px-4 py-2 border'>{returnOrder.type}</td>
                                <td className='px-4 py-2 border'>{returnOrder.status}</td>
                                <td className='px-4 py-2 border'>{returnOrder.reason}</td>
